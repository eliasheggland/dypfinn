...
function renderMarkers(){
  ...
  ordered.forEach(a=>{
    ...
    const html=labelled?`<div class="spot-label ${state.selected===a.id?'selected':''}"><span class="score">${icon('fish')}</span><span><span class="spot-fish">${a.kind||'Egen plass'}</span><span class="spot-depth">${depthLabel(a.depth)}</span></span></div>`:`<div class="fish-pin ${db.saved.includes(a.id)?'saved':''}">${icon('fish')}</div>`;
...
function renderDetail(){
  ...
  const why=a.custom?'Egen kartplass. Kartverket oppgir dybdeintervallet nedenfor. Terrenget rundt er ikke analysert.':a.analysis?.summary||`${a.kind} med ${depthLabel(a.depth)} ved markøren. Grovt kartlagt område fra en tidligere analyse.`;
  $('#detail-panel').hidden=false;$('#detail-panel').innerHTML=`<button class="sheet-handle mobile-only" data-action="expand-detail" aria-label="Utvid eller minimer stedsdetaljer"><span></span></button><div class="detail-header"><div><p class="eyebrow">${a.custom?'DIN PRIVATE PLASS':'FUNNET FRA SJØKART · FISK IKKE BEKREFTET'}</p><h2>${esc(a.name)}</h2><p class="detail-subtitle">${esc(a.region)} · ${formatDistance(a.distance)} fra søkesenter</p></div><button class="icon-button" data-action="close-detail" aria-label="Lukk stedsdetaljer">${icon('close')}</button></div>
  <div class="detail-scroll"><div class="detail-hero"><div class="depth-reading"><span>Bunndyp ved markøren</span><strong>${depthLabel(a.depth)}</strong></div>${score?`<div class="score-box"><strong>${a.kind==='Toppkant'?'Topp':'Kant'}</strong><small>BUNNFORM</small></div>`:''}</div><p class="source-caption">${a.depth?'Kartverket · dybdeintervall i sjøkartet':'Ingen tilgjengelige dybdedata for punktet'}</p>
  <div class="weather-inline" id="spot-weather">${weatherInline()}</div><h3 class="section-title">Mulige arter <span>BEREGNET FRA DYBDE</span></h3><div class="fish-tags">${a.choices.map(c=>`<button class="fish-tag" data-guide="${c.species.id}">${c.species.name}</button>`).join('')||'<span class="muted">Kan ikke vurderes uten dybdedata</span>'}</div>
  <h3 class="section-title">Hvorfor akkurat her?</h3><p class="body-copy">${why}</p>${spotEvidence(a)}<div class="data-grid">${tile('Bunnform',esc(a.kind))}${tile('Bunntype','Ikke kartlagt')}${tile('Dybdeforskjell',a.analysis?'Minst '+a.analysis.drop+' m':'Ikke bekreftet')}${tile('Kartprøver',a.analysis?a.analysis.sampleCount+' av 9':'Begrenset')}</div>
  <h3 class="section-title">Bunnprofil <span>${icon('layers')}</span></h3>${terrainChart(a)}
  ${fish?`<h3 class="section-title">Minstemål og regler</h3>${ruleCard(fish.id,a.coordinates[0])}<h3 class="section-title">Slik leter du etter ${fish.name.toLowerCase()}</h3><p class="body-copy">${fish.technique}</p><div class="data-grid">${tile('Agn / lokkemiddel',fish.bait)}${tile('Metode',fish.rig)}</div>${fish.pelagic?'<p class="hint">Fiskedybden avhenger av stimene. Bunndypet er vist på kartet.</p>':''}`:''}
  <button class="secondary grow" data-action="drift">${icon('route')}Planlegg drift over området</button>
  <details><summary>Hvordan beregnes vurderingen?</summary><div class="score-bars">${score?Object.entries({Dybde:score.depth,Bunnform:score.structure,Terreng:score.terrain}).map(([k,v])=>`<div class="score-bar"><span>${k}</span><i><span style="width:${v}%"></span></i><b>${v}</b></div>`).join(''):''}</div><p class="hint">Vurderingen sammenligner dybde og bunnform med det arten vanligvis foretrekker. Vær, agnfisk, strøm og fangster inngår ikke. Tallet er ikke en fangstsjanse.</p></details>
  <h3 class="section-title">Posisjon <span>WGS 84</span></h3>${a.nearby?`<p class="body-copy">${formatDistance(a.nearby.distance)} ${a.nearby.direction} for ${esc(a.nearby.name)}. Stedsreferanse fra Kartverket.</p>`:''}<p class="body-copy num">${coords(a.coordinates)}</p><div class="inline-actions"><button class="text-link" data-action="copy-coordinates">Kopier koordinater</button><button class="text-link" data-action="share-spot">Del plassen</button><button class="text-link" data-action="navigate">Avstand og retning ${icon('arrow')}</button></div>
  <p class="detail-note">Dybder er kartintervaller. Terreng og arter er grove vurderinger; fiskeforekomst og bunntype er ikke bekreftet.${a.depth?.updated?` Kartobjekt oppdatert ${esc(a.depth.updated.slice(0,10))}; dette er ikke en ny dybdemåling.`:''} <button class="text-link" data-action="sources">Se datakilder</button></p><p class="hint">Sjekk minstemål, fredning og lokale begrensninger før fiske. Appen viser utvalgte artsregler, men kontrollerer ikke alle lokale fredningsområder. <a href="https://www.fiskeridir.no/fritidsfiske" target="_blank" rel="noopener">Fiskeridirektoratets regler</a></p></div>
...
function showFilters(){
  ...
  <label class="field">Bunnform<select name="kind">${option('all','Alle bunnformer',state.kind)}${['Dypkant','Rennekant','Toppkant','Dybdeovergang'].map(v=>option(v,v,state.kind)).join('')}</select></label>
...
function locate(){if(!navigator.geolocation){toast('Posisjon støttes ikke i denne nettleseren.');return;}toast('Henter posisjonen din…');navigator.geolocation.getCurrentPosition(p=>{const coordinates=[p.coords.latitude,p.coords.longitude];state.location=coordinates;state.center=coordinates;locationLayer.clearLayers();L.circle(coordinates,{radius:p.coords.accuracy,color:'#93c5ff',weight:1,fillOpacity:.12}).addTo(locationLayer);L.circleMarker(coordinates,{radius:7,color:'#fff',weight:2,fillColor:'#5c9fec',fillOpacity:1}).addTo(locationLayer);map.setView(coordinates,13);renderResults();loadConditions(coordinates);toast(`Posisjon funnet · nøyaktighet ca. ${Math.round(p.coords.accuracy)} m`);},()=>toast('Posisjonen er utilgjengelig. Tillat posisjon i nettleseren, eller utforsk kartet manuelt.'),{enableHighAccuracy:true,timeout:12000,maximumAge:60000});}
...
const actions={
  ...
  'close-dialog':closeModal,'close-detail':closeDetail,'species':showSpecies,'filters':showFilters,'layers':showLayers,'fit':fitResults,'locate':locate,'sources':showSources,'conditions':showConditions,'analyze':analyze,
...
async function init(){
  ...
  let pressTimer,touchStart;$('#map').addEventListener('touchstart',e=>{if(e.touches.length!==1)return;touchStart=[e.touches[0].clientX,e.touches[0].clientY];const r=$('#map').getBoundingClientRect();pressTimer=setTimeout(()=>{const p=map.containerPointToLatLng([touchStart[0]-r.left,touchStart[1]-r.top]);inspectPoint([p.lat,p.lng]);},650);},{passive:true});
